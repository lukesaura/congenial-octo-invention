// src/components/ParkingViolations.tsx
import { useEffect, useState } from 'react';
import { Car, Calendar, MapPin, User, CheckCircle2 } from 'lucide-react';
...
export function ParkingViolations() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const items: ParkingViolation[] = snap.docs.map(d => {
  ...
  if (loading) {
    return <div className="py-8 text-center text-gray-600">Loading parking violations...</div>;
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0">
        <div>
          <h2 className="text-gray-800 text-2xl font-semibold">Parking Violations</h2>
          <p className="text-gray-500 mt-1 text-sm">Monitor and approve parking violation reports</p>
        </div>
      ...
      <div className="space-y-3">
        {violations.map((violation) => (
          <div
            ...
            } ${expandedId === violation.id ? 'shadow-lg' : 'shadow-sm hover:shadow-md'}`}
          >
            <button
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
                  <div className={`flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex-shrink-0 ${
                    violation.approved ? 'bg-green-100' : 'bg-red-100'
                  }`}>
                    <Car className={`w-5 h-5 sm:w-6 sm:h-6 ${violation.approved ? 'text-green-600' : 'text-red-600'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <h3 className="text-gray-800 text-sm sm:text-base">{violation.licensePlate}</h3>
                      {violation.approved && (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs sm:text-sm">
                      <div className="flex items-center gap-2 text-gray-600 truncate">
                        ...
                        <span className="truncate">{violation.vehicleOwner}</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-600">
                        <Calendar className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                        <span className="text-xs sm:text-sm">{violation.dateTime}</span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm flex-shrink-0 ${
                  ...
                  {violation.approved ? 'Approved' : 'Pending'}
                </div>
              </div>
            </button>
              ...
              <div className="px-4 sm:px-5 pb-4 sm:pb-5 border-t border-gray-200 pt-4 bg-white">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
                  <div className="bg-gray-50 rounded-lg p-3">
                    <p className="text-gray-600 text-xs sm:text-sm mb-1">Spot Owner</p>
                    <p className="text-gray-800 text-sm sm:text-base">{violation.spotOwner}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                    <p className="text-gray-600 text-xs sm:text-sm mb-1">Plot Number</p>
                    <p className="text-gray-800 text-sm sm:text-base flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-blue-600" />
                      {violation.plotNumber}
                    </p>
                  </div>
                </div>
                {violation.approved && violation.approvedAt && (
                  <p className="text-green-600 text-xs sm:text-sm mb-4">Approved on {violation.approvedAt}</p>
                )}
                {!violation.approved && (
                  <button
                    ...
                    disabled={approvingId === violation.id}
                  >
                    <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5" />
                    {approvingId === violation.id ? 'Approving...' : 'Approve Violation'}
                  </button>
                )}
              </div>
            )}
